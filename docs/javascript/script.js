...
function insertProject(containerId, project) {
  ...
  const projectCard = $('<div>', {
    class: 'card  shadow-sm overflow-hidden m-4 flex flex-col animate__animated animate__fadeIn',
  })
  ...
  project.topics.forEach((topic) => {
    const topicElement = $('<span>', {
      class: 'topic-item  text-xs md:text-sm mr-2 px-2 py-1 m-2 my-bg-quaternary-color',
      text: topic,
    })
...
function insertDesign(containerId, imageSrc, smallerImages, title, description) {
...
function insertVideo(containerId, videoLink, title, description) {
  const cardContainer = document.createElement('div')
  cardContainer.classList.add('card', 'rounded-lg', 'overflow-hidden', 'shadow-sm', 'mx-4')
